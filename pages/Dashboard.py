...
with tab4:
    ...
    if not subs_data.data:
        ...
        # ── Bulk fetch assignments + topics + subjects (no N+1 queries) ──
        _all_assignments = sb.table("assignments").select("id, title, difficulty, topic_id").execute()
        _all_topics      = sb.table("topics").select("id, title, subject_id").execute()
        _all_subjects    = sb.table("subjects").select("id, name").execute()
        _assign_map  = {a["id"]: a for a in _all_assignments.data}
        _topic_map   = {t["id"]: t for t in _all_topics.data}
        _subject_map = {s["id"]: s for s in _all_subjects.data}

        # ── Status filter row ─────────────────────────
        _fcol, _, _ccol = st.columns([2, 3, 2])
        with _fcol:
            _sf = st.selectbox("Filter by status",
                               ["All", "✅ Passed", "❌ Failed", "⏳ Pending"],
                               key="act_filter")
        _sf_map = {"✅ Passed":"passed","❌ Failed":"failed","⏳ Pending":"pending"}

        recent = sorted(subs_data.data,
                        key=lambda x: x.get("submitted_at",""), reverse=True)[:15]
        if _sf != "All":
            recent = [s for s in recent if s.get("status") == _sf_map.get(_sf)]

        with _ccol:
            _pc = len([s for s in subs_data.data if s.get("status")=="passed"])
            _fc = len([s for s in subs_data.data if s.get("status")=="failed"])
            _nc = len([s for s in subs_data.data if s.get("status") not in ("passed","failed")])
            st.markdown(
                '<div style="display:flex;gap:8px;margin-top:8px;justify-content:flex-end;">' +
                '<span style="background:rgba(34,197,94,0.15);color:#4ade80;border:1px solid rgba(34,197,94,0.3);padding:3px 10px;border-radius:20px;font-size:12px;font-weight:700;">✅ ' + str(_pc) + '</span>' +
                '<span style="background:rgba(239,68,68,0.15);color:#f87171;border:1px solid rgba(239,68,68,0.3);padding:3px 10px;border-radius:20px;font-size:12px;font-weight:700;">❌ ' + str(_fc) + '</span>' +
                '<span style="background:rgba(245,158,11,0.15);color:#fbbf24;border:1px solid rgba(245,158,11,0.3);padding:3px 10px;border-radius:20px;font-size:12px;font-weight:700;">⏳ ' + str(_nc) + '</span></div>',
                unsafe_allow_html=True)

        st.write("")
        if not recent:
            st.info(f"No {_sf.lower()} submissions found.")
        else:
            for _idx, sub in enumerate(recent):
                _a = _assign_map.get(sub["assignment_id"])
                if not _a:
                    continue
                _topic   = _topic_map.get(_a.get("topic_id",""), {})
                _subject = _subject_map.get(_topic.get("subject_id",""), {})
                _subj_name  = _subject.get("name", enrolled[0] if enrolled else "")
                _topic_name = _topic.get("title", "")

                _status   = sub.get("status", "pending")
                _score    = sub.get("score", 0)
                _feedback = sub.get("feedback", "") or ""
                _submitted = sub.get("submitted_at","") or sub.get("created_at","") or ""
                try:
                    _dt  = datetime.fromisoformat(_submitted.replace("Z",""))
                    _dts = _dt.strftime("%d %b %Y, %I:%M %p")
                except:
                    _dts = _submitted[:16] if _submitted else "—"

                # Status config
                _scfg = {
                    "passed":  ("✅","#4ade80","rgba(34,197,94,0.12)","rgba(34,197,94,0.3)","PASSED"),
                    "failed":  ("❌","#f87171","rgba(239,68,68,0.12)","rgba(239,68,68,0.3)","FAILED"),
                    "pending": ("⏳","#fbbf24","rgba(245,158,11,0.12)","rgba(245,158,11,0.3)","PENDING"),
                }.get(_status, ("⏳","#fbbf24","rgba(245,158,11,0.12)","rgba(245,158,11,0.3)","PENDING"))
                _si, _sc, _sbg, _sbdr, _slbl = _scfg

                # Difficulty config
                _dcfg = {
                    "Easy":   ("#4ade80","rgba(34,197,94,0.12)","rgba(34,197,94,0.25)"),
                    "Medium": ("#fbbf24","rgba(245,158,11,0.12)","rgba(245,158,11,0.25)"),
                    "Hard":   ("#f87171","rgba(239,68,68,0.12)","rgba(239,68,68,0.25)"),
                }.get(_a.get("difficulty",""), ("#a5b4fc","rgba(102,126,234,0.12)","rgba(102,126,234,0.25)"))
                _dc, _dbg, _dbdr = _dcfg

                _score_pct   = min(int((_score/10)*100), 100)
                _score_color = "#4ade80" if _status=="passed" else ("#f87171" if _status=="failed" else "#fbbf24")

                # Build card HTML as plain string — no nested f-strings
                _topic_pill = (
                    '<span style="background:rgba(168,85,247,0.12);color:#c084fc;' +
                ...
                _feedback_html = (
                    ...
                    'color:rgba(200,208,231,0.6);border:1px solid rgba(255,255,255,0.05);">' +
                    '💬 ' + str(_feedback)[:120] + ("…" if len(str(_feedback))>120 else "") +
                    '</div>'
                ) if _feedback else ""
                ...
                _card = (
                    ...
                    'border-radius:16px;padding:1.1rem 1.3rem;margin-bottom:0.6rem;">' +

                    '<div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:8px;margin-bottom:8px;">' +
                    '<div style="flex:1;">' +
                    '<div style="color:white;font-weight:700;font-size:15px;margin-bottom:4px;">' +
                    _si + ' ' + _a["title"] +
                    '</div>' +
                    '<div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;">' +
                    '<span style="background:rgba(102,126,234,0.15);color:#a5b4fc;' +
                    'border:1px solid rgba(102,126,234,0.3);' +
                    'padding:2px 9px;border-radius:20px;font-size:11px;font-weight:600;">' +
                    (_subj_name or "—") + '</span>' +
                    _topic_pill +
                    '<span style="background:' + _dbg + ';color:' + _dc + ';' +
                    'border:1px solid ' + _dbdr + ';' +
                    'padding:2px 9px;border-radius:20px;font-size:11px;font-weight:600;">' +
                    _a.get("difficulty","") + '</span>' +
                    '</div></div>' +
                    '<span style="background:' + _sbg + ';color:' + _sc + ';' +
                    'border:1px solid ' + _sbdr + ';' +
                    'padding:4px 12px;border-radius:20px;font-size:12px;font-weight:700;' +
                    'white-space:nowrap;">' + _si + ' ' + _slbl + '</span>' +
                    '</div>' +

                    '<div style="margin-bottom:8px;">' +
                    '<div style="display:flex;justify-content:space-between;margin-bottom:3px;">' +
                    '<span style="font-size:12px;color:rgba(200,208,231,0.5);">Score</span>' +
                    '<span style="font-size:12px;font-weight:700;color:' + _score_color + ';">' +
                    str(_score) + '/10</span></div>' +
                    '<div style="background:rgba(255,255,255,0.07);border-radius:999px;height:6px;">' +
                    '<div style="width:' + str(_score_pct) + '%;height:100%;border-radius:999px;' +
                    'background:' + _score_color + ';"></div></div></div>' +

                    _feedback_html +

                    '<div style="font-size:11px;color:rgba(200,208,231,0.35);">🕒 ' + _dts + '</div>' +
                    '</div>'
                )
                ...
                with _bc:
                    _btn_lbl = "🔁 Retry" if _status=="failed" else ("📝 Review" if _status=="passed" else "📝 Open Assignment")
                    if st.button(_btn_lbl, key=f"nav_a_{_idx}", use_container_width=True):
                        st.session_state["nav_subject"] = _subj_name
                        st.session_state["nav_topic"]   = _topic_name
                        st.session_state["nav_page"]    = "Assignments"
